...
<div id="content">
    <div class="page-header">
        <div class="container-fluid">
            <div class="pull-right"><a href="<?php echo $common->pageUrl('users/index_form') ?>" data-toggle="tooltip" title="Add New" class="btn btn-primary"><i class="fa fa-plus"></i></a>
                        ...
                        <table class="table table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <td class="text-left"><a <?php if (isset($request->get['sort'])) {
            if ($request->get['sort'] == 'name') {
                echo $order_class;
            }
        } ?> href="<?php $common->pageUrl('users/index'); ?>?sort=name<?php echo $order_by . $page_no . $filter_string; ?>">Name</a></td>
                                    <td class="text-left"><a <?php if (isset($request->get['sort'])) {
            if ($request->get['sort'] == 'email') {
                echo $order_class;
            }
        } ?> href="<?php $common->pageUrl('users/index'); ?>?sort=email<?php echo $order_by . $page_no . $filter_string; ?>">Email</a></td>
                                    <td class="text-right"><a <?php if (isset($request->get['sort'])) {
            if ($request->get['sort'] == 'mobile_no') {
                echo $order_class;
            }
        } ?> href="<?php $common->pageUrl('users/index'); ?>?sort=mobile_no<?php echo $order_by . $page_no . $filter_string; ?>">Mobile Number</a></td>
                                    <td class="text-right"><a <?php if (isset($request->get['sort'])) {
            if ($request->get['sort'] == 'created') {
                echo $order_class;
            }
        } ?> href="<?php $common->pageUrl('users/index'); ?>?sort=created<?php echo $order_by . $page_no . $filter_string; ?>">Date Added</a></td>
                                    <td class="text-left"><a <?php if (isset($request->get['sort'])) {
            if ($request->get['sort'] == 'status') {
                echo $order_class;
            }
        } ?> href="<?php $common->pageUrl('users/index'); ?>?sort=status<?php echo $order_by . $page_no . $filter_string; ?>">Status</a></td>
                                    <td class="text-left"><a <?php if (isset($request->get['sort'])) {
            if ($request->get['sort'] == 'country') {
                echo $order_class;
            }
        } ?> href="<?php $common->pageUrl('users/index'); ?>?sort=country<?php echo $order_by . $page_no . $filter_string; ?>">Country</a></td>
                                    <td class="text-left"><a <?php if (isset($request->get['sort'])) {
                                    if ($request->get['sort'] == 'device_name') {
                                        echo $order_class;
                                    }
                                } ?> href="<?php $common->pageUrl('users/index'); ?>?sort=device_name<?php echo $order_by . $page_no . $filter_string; ?>">Device</a></td>
    ...
    foreach ($results as $result) {
        ?>
                                        <tr>
                                            ...
                                            <td class="text-left"><img src="<?php $img = (!empty($result['profile_img'])) ? PROFILE_IMG . $result['profile_img'] : '';
...
<?php echo pagination($statement, $limit, $page, $common->pageUrl('users/index', true) . '?' . $order_paging . $sort_by . $filter_string . '&'); ?>
...
$('#button-filter').on('click', function() {
        url = '<?php echo $common->pageUrl('users/index', true) . '?' . $order_paging . $sort_by; ?>';
