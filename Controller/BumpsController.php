 ...
 */
			...
			foreach ($bumped_users as $key => $b_user) {
				...
				if(empty($already_bumped)){
	    			...
	    			$this->Notification->save($notification);
                                ...
                                // new JSON packet for encounter android 
                                $bump_array = array();
                                $bump_array["me_id"] = $my_id;
                                $bump_array["message"] = "You've had a close encounter!";
                                $bump_array["with"] = $bumped_users[$key]['User']['id'];
                                // for andrid new push only bump format of JSON
                                $this->_android_push(json_encode($bump_array), $my_id);
	...
	public function bumped_list($my_id){
		...
		if(!empty($my_check)){
            ...
            $my_block_list = $this->Bump->query('SELECT userId1 FROM blocks WHERE userId2 = '.$my_id.';');
			...
			die(json_encode(array('success' =>true, 'my_bumped_list' => $my_bumped_list, 'my_block_list' => $my_block_list)));
